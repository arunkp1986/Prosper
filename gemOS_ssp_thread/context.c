...
int exec_init(struct exec_context *ctx, struct init_args *args)
{
   ...
   if(!ptr_dirtymap){
       ...
       //dprintk("dirty num bytes: %u, dirty num pages: %u, dirty bitmap area: ptr:%x\n",dirty_num_bytes,dirty_num_pages,ptr_dirtymap);
   ...
   writemsr(MISCREG_TRACK_START, (u64)(STACK_START - MAX_STACK_SIZE));
...
void copy_mm(struct exec_context *child, struct exec_context *parent)
{
  ...
  for(vaddr = seg->end - PAGE_SIZE; vaddr >= seg->next_free; vaddr -= PAGE_SIZE){
     ...
     if(parent_pte){
           u64 pfn = install_ptable((u64)os_addr, seg, vaddr, 0, 1);  //Returns the blank page in nvm  
           pfn = (u64)osmap(pfn);
           memcpy((char *)pfn, (char *)(*parent_pte & FLAG_MASK), PAGE_SIZE); 
     }
  } 
