...
//u8 mapping_table_pos = 0;
u64 * backmapping = NULL;
int exec_init(struct exec_context *ctx, struct init_args *args)
{
   ...
   bzero((char *)os_addr, PAGE_SIZE);
   textpfn = install_ptable_multi((u64) os_addr, ctx->mms[MM_SEG_CODE].start, CODE_PAGES, 0,0);//allocated in nvm
   ctx->mms[MM_SEG_CODE].next_free = ctx->mms[MM_SEG_CODE].start + (CODE_PAGES << PAGE_SHIFT);
   if(!backmapping){
       unsigned num_bytes = (get_pages_region(NVM_USER_REG)*sizeof(struct mapping_entry));
       unsigned num_pages = num_bytes%PAGE_SIZE?(num_bytes>>PAGE_SHIFT)+1:num_bytes>>PAGE_SHIFT;
       //printk("num pages:%u\n",num_pages);
       backmapping = (u64*)os_page_alloc_pages(FILE_STORE_REG,num_pages);
   }

   install_ptable((u64) os_addr, &ctx->mms[MM_SEG_DATA], 0, 0, 0); //allocates in nvm   
