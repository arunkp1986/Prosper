...
static u32 get_free_pages(struct page_list *pgl,u32 num)
{
  u32 pfn, bit;
  u32 first_pfn = 0;
  u32 size_in_u32 = pgl->size >> 8, once = 0; //changed to 5 from 8
  ...
  for(loop_index=0; loop_index<num; loop_index++){
      pfn = (pgl->last_free_pos << 5) + (bit+loop_index);
      if(!first_pfn){
          first_pfn = pfn;
      }
...
u32 os_pfn_alloc(u32 region)
{
   ...
   if(region == NVM_USER_REG && backmapping ){
       ...
       pfn_back = get_free_page(pgl);
       entry->maddr = (unsigned long)pfn << PAGE_SHIFT;
       entry->baddr = (unsigned long)pfn_back << PAGE_SHIFT;
       //printk("maddr:%lx, baddr:%lx\n",entry->maddr,entry->baddr);
   }
...
void init_os_chunk_alloc(u32 region){
    ...
    for(int i=0; i<7; i++){
        n_meta->nugget_size[i] = (1<<(5+i));
	init_list_head(&n_meta->size_list[i]);
        struct osalloc_nugget * ch_new = (struct osalloc_nugget*)os_page_alloc(region);
        ch_new->is_full = 0;
        ch_new->chunk_size = (1<<(5+i));
        ch_new->current_pfn = os_pfn_alloc(region);
        for(int i=0; i<4; i++){
            *((u32*)ch_new->bitmap+i) |= ~(1UL<<32);
        }
        list_add_tail(&ch_new->list,&n_meta->size_list[i]);
	if(region == NVM_META_REG){
	    clflush_multiline((u64)ch_new,sizeof(struct osalloc_nugget));
	    clflush_multiline((u64)&n_meta->size_list[i],sizeof(struct list_head));
	}
    }
...
struct osalloc_nugget * get_new_os_chunk(u32 size, u32 region){
    ...
    u32 num_nugget_structs = PAGE_SIZE>>i;
	...
	if(nug_meta_temp->region == region){
            for(int i=0; i<7; i++){
	        if(nug_meta_temp->nugget_size[i] == size){
		    struct list_head *nug_temp = (nug_meta_temp->size_list[i]).next;
		    u32 pos = 1;
                    while(nug_temp->next != &nug_meta_temp->size_list[i]){
                        pos +=1;
	                ...
	                ch_new = (struct osalloc_nugget*)((u64)list_entry(nug_temp, struct osalloc_nugget, list)+
				       sizeof(struct osalloc_nugget));
                        ...
                        list_add_tail(&ch_new->list,&nug_meta_temp->size_list[i]);
			if( region == NVM_META_REG ){
		            clflush_multiline((u64)ch_new, sizeof(struct osalloc_nugget));
		            clflush_multiline((u64)&nug_meta_temp->size_list[i], sizeof(struct list_head));
			}
			return ch_new;
		    }
                    else{
...
u64 os_chunk_alloc(u32 size, u32 region){
	...
	if(nug_meta_temp->region == region){
            for(int i=0; i<7; i++){
	        if(nug_meta_temp->nugget_size[i] == alloc_size){
		    list_for_each(nug_head_temp, &nug_meta_temp->size_list[i]){
		        ch_new = list_entry(nug_head_temp, struct osalloc_nugget, list);
				    ...
				    if( region == NVM_META_REG ){
				        clflush_multiline((u64)ch_new, sizeof(struct osalloc_nugget));
		                    ...
		                    retval = (u64)(ch_new->current_pfn<<PAGE_SHIFT)+\
					(loop_counter*alloc_size);
			            return retval;
			        }
                                loop_counter += 1;
			    }
