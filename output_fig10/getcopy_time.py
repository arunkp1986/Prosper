...
if __name__=="__main__":
    ...
    with open('result_checkpoint_time.csv', 'w', newline='') as csvfile:
        fieldnames = ['bench', '128-byte','64-byte','32-byte','16-byte','8-byte']
        ...
        for k1,v1 in final_data.items():
            if(k1 in benchmarks):
                ...
                for k2,v2 in v1.items():
                    #print(final_data[k1]["4096"][para])
                    #d[k2] = round(v2[para]/v2["number"])
                    if( k2 != "4096"):
                        key = k2+"-byte"
                        d[key] = round((v2[para]/v2["number"])*1.000000000/(final_data[k1]["4096"][para]/final_data[k1]["4096"]["number"]),4)
                writer.writerow(d)
