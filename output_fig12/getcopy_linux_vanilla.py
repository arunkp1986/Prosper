...
def get_files(dirname, files):
    for (dirpath, dirnames, filenames) in os.walk(cwd):
        if (cwd+'/linux_vanilla/'+dirname in dirpath):
...
if __name__=="__main__":
    ...
    for k1,v1 in final_data.items():
        with open('{0}_result_vanilla.csv'.format(k1), 'w', newline='') as csvfile:
            ...
            for k2,v2 in v1.items():
                if(k2 == "0bytes"):
                    for k3,v3 in v2.items():
                        if(k3 =="hwsw"):
                            writer.writerow({'size':"vanilla",'numUsrInsts':v3["numUsrInsts"],'numUsrCycles':v3["numUsrCycles"]})   
