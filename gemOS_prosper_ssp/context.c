...
int exec_init(struct exec_context *ctx, struct init_args *args)
{
   ...
   if(!ptr_sspmap){
       ...
       unsigned dirty_num_pages = dirty_num_bytes>>PAGE_SHIFT;
       dirty_num_pages = (dirty_num_pages<<PAGE_SHIFT == dirty_num_bytes)?dirty_num_pages:dirty_num_pages+1;
       ptr_sspmap = (struct ssp_entry*)os_page_alloc_pages(NVM_META_REG,dirty_num_pages);
       writemsr(MISCREG_SSP_ADDR, (u64)ptr_sspmap);
       //printk("dirty num bytes: %u, dirty num pages: %u, dirty bitmap area: ptr:%x\n",dirty_num_bytes,dirty_num_pages,ptr_sspmap);
   ...
   writemsr(MISCREG_SSP_START, (u64)MMAP_START);
   writemsr(MISCREG_SSP_END, (u64)MMAP_AREA_END);
