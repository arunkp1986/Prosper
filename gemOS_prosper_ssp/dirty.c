...
void start_track(struct exec_context* ctx, u16 track_size){
	...
	//printk("dirty bitmap area, only once, ptr:%x, pages:%u\n",ptr_dirtymap,dirty_num_pages);
    ...
    dprintk("ptr:%x gran:%u\n",ptr_dirtymap,LOG_TRACK_GRAN);
    writemsr(MISCREG_TRACK_START, (u64)stack_start);
    //writemsr(MISCREG_TRACK_START, (u64)0);
    writemsr(MISCREG_TRACK_END, (u64)stack_end);
    //writemsr(MISCREG_TRACK_END, (u64)0);
...
void alloc_se_dirty_bitmap(struct exec_context *ctx, u64 start, u64 end, u16 track_size){
    ...
    if(extra_entries > 1000)
        extra_entries = 1000;

    //printk("current entries: %d extra_entries:%d \n",current_entries,extra_entries);
    while(extra_entries){
...
u32 read_dirty_bitmap(struct exec_context *ctx, u64 start, u64 end, u16 track_size){
	...
	printk("min address not set in gem5\n");
        return 0;
    }
    unsigned long stack_start = ctx->mms[MM_SEG_STACK].start;
    ...
    if(start_bit_pos<0){
        printk("Bug!!! at %s\n",__func__);
	printk("start:%lx, stack_start:%lx\n",start, stack_start);
    }
